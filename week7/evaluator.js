...
export class Evaluator {
    ...
    Program(node) {
        return this.evaluateModule(node.children[0]);
    }
    IFStatement(node) {
        let condition = this.evaluateModule(node.children[2]);
        if (condition instanceof Reference)
            condition = condition.get();
        if (condition.toBoolean().value) {
            return this.evaluateModule(node.children[4]);
        }
    }
    WhileStatement(node) {
        while (true) {
    ...
    MemberExpression(node) {
        ...
        if(node.children.length === 3) {
            debugger;
            let obj = this.evaluateModule(node.children[0]).get();
    ...
    Arguments(node) {
        if(node.children.length === 2)
            return [];
        else return this.evaluateModule(node.children[1]);
    }
    ArgumentList(node) {
        if(node.children.length === 1) {
            let result = this.evaluateModule(node.children[0]);
            if(result instanceof Reference)
                result = result.get();
            return [result];
        } else {
