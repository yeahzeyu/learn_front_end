...
第六课：
（toy browser离真正的浏览器还差很多，甚至我们也没有去做文字渲染，也没有去做边框，也没有去做layout的offset的逻辑。主要是用来理解整个浏览器的工作原理）
递归调用子元素的绘制方法完成DOM树的绘制
忽略一些不需要绘制的节点
实际浏览器中，文字绘制是难点，需要依赖字体库，我们这里忽略
实际浏览器中，还会对一些涂层做compositing，我们这里也忽略了

总结：
从开局只有一个URL，把它变成了一段HTML代码，再从HTML代码又把它变成了一棵DOM树，又把这棵什么都没有的DOM树加上了CSS的属性，再把这些CSS属性最终计算出来它的每一个元素的位置，直到最后我们在一张图片上把网页的内容给它画了出来，这个过程已经是一个浏览器从一个URL到最终呈现的一个最基础的答案了。通过开发toy browser，对浏览器的工作原理已经有了一个深入的理解和感性的认知了。
